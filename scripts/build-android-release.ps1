# Build van1 Android release APK (merchant app).
...
if ($usingDebugSigning -and -not $AllowDebugSigning) {
    Write-Error @"
Missing android/key.properties — copy from van2/android or key.properties.example before store release.
...
$apkDst = Join-Path $releasesDir "van1-$version-release.apk"
