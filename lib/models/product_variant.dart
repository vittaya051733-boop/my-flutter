...
class ProductVariantSupport {
  ...
  static List<String> readProductImageUrls(Map<String, dynamic> data) {
    return (data['imageUrls'] as List?)
            ?.whereType<String>()
            .map((url) => url.trim())
  ...
  static List<String> readProductThumbnailUrls(Map<String, dynamic> data) {
    final thumbnails = (data['thumbnailUrls'] as List?)
            ?.whereType<String>()
            .map((url) => url.trim())
            .where((url) => url.isNotEmpty)
            .toList(growable: false) ??
    ...
    return readProductImageUrls(data);
  }
  ...
  static List<ProductVariant> variantsForImageIndex(
    ...
    final originals = readProductImageUrls(productData);
    final thumbs = readProductThumbnailUrls(productData);
    final candidates = <String>{
      if (imageIndex < originals.length) originals[imageIndex],
      if (imageIndex < thumbs.length) thumbs[imageIndex],
  ...
  static String formatPriceRange(Iterable<double> prices) {
    final values = prices.where((price) => price > 0).toList(growable: false);
    if (values.isEmpty) {
      return '';
    }
    ...
    if (min == max) {
      return _formatBaht(min);
    }
    return '${_formatBaht(min)}–${_formatBaht(max)}';
  }

  static String _formatBaht(double value) {
  ...
  static int sumStock(Iterable<ProductVariant> variants) {
    return variants.fold<int>(0, (sum, variant) => sum + variant.stock);
  }

  static String buildVariantOptionsLine(
    List<ProductVariant> variants, {
    required String Function(String storedColor) colorLabel,
  }) {
    final colors = uniqueOptionValues(variants, colors: true);
    final sizes = uniqueOptionValues(variants, colors: false);
    final parts = <String>[];
    if (colors.isNotEmpty) {
      parts.add('สี: ${colors.map(colorLabel).join(' · ')}');
    }
    if (sizes.isNotEmpty) {
      parts.add('ขนาด: ${sizes.join(' · ')}');
    }
    return parts.join('  ');
  }
