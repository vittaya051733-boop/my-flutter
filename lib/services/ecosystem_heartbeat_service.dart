...
class EcosystemHeartbeatService {
  ...
  static const String appId = 'van1';
  static const String notificationTargetApp = 'van1';
    ...
    try {
      ...
      Future<void> check(String pointId, Future<void> Function() action) async {
        try {
          ...
          debugPrint('van1 heartbeat $pointId failed: $error');
        }
      }
      ...
      await check('V1-CREDITS', () async {
        await AppCheckGuard.ensureFinancialReady();
        await _functions.httpsCallable('getMerchantWallet').call(
          <String, dynamic>{'merchantUid': uid},
        );
      });
      await check('V1-NOTIF', () async {
        await AppCheckGuard.ensureFinancialReady();
        await db
            .collection('app_notifications')
            .where('targetApp', isEqualTo: notificationTargetApp)
      ...
      debugPrint('van1 heartbeat write failed: $error');
    } finally {
      _pulsing = false;
    }
  }
