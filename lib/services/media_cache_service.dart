...
/// This allows the app to reuse freshly uploaded media instantly without
/// performing another network download.
class MediaCacheService {
  ...
  /// Copy [source] into the cache directory for [bucket] and associate it with [url].
  /// Returns the cached file when successful.
  Future<File?> cacheUploadedFile({
    ...
    try {
      ...
      final extension = p.extension(source.path).isNotEmpty ? p.extension(source.path) : '.bin';
  ...
  /// Returns the cached file path for [url] if it still exists on disk.
  ...
  Future<Directory> _ensureBucketDir(MediaCacheBucket bucket) async {
    final baseDir = await getApplicationDocumentsDirectory();
    final dir = Directory(p.join(baseDir.path, 'media_cache', _bucketName(bucket)));
    if (!await dir.exists()) {
